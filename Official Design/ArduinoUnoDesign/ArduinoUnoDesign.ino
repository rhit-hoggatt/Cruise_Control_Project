...
//Inputs
int speed_signal = 8;     //analog signal (sinusoid) requires pin 49 for FreqCount lib (pin 8 for UNO)
int brake_signal = 9;      //0v when off 12v when brake pressed (A1)
bool brake_signal_state;
int cancel_signal = 10;     //12v when off 0v when cancel pressed (A2)
bool cancel_signal_state;
int resume_signal = 11;     //0v until 12v when resume pressed
bool resume_signal_state;
int set_signal = 12;        //0v until 12v when pressed
bool set_signal_state;
int clutch_signal = 13;     //0v until 12v when clutch pressed
